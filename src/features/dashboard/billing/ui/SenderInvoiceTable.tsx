...
interface Props {
  ...
  fetchInvoices: (offset: number) => void;
}

export default function SenderInvoiceTable({
  invoices,
  loadingOrders,
  fetchInvoices,
}: Props) {
  ...
  const rows = invoices?.results.map((row, index) => (
    <Table.Tr key={index}>
      ...
      <Table.Td>{row.owner.company}</Table.Td>
      <Table.Td>
      ...
      <Table.Td>{moment(row.due_date).format("Do MMM YYYY")}</Table.Td>
